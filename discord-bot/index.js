...
client.on('messageCreate', async (message) => {
  ...
  */
  ...
  new SlashCommandBuilder()
    .setName('duel')
    .setDescription('Reta a un jugador')

    .addUserOption((option) =>
      option
        .setName('jugador')
        .setDescription('Jugador retado')
        .setRequired(true),
    )
    ...
    .addIntegerOption((option) =>
      option
        .setName('ft')
        .setDescription('Cantidad de partidas')
        .setRequired(true),
    );
  ...
  if (interaction.commandName === 'duel') {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `${challenger} desafía a ${opponent} a un **FT${ft}**.\n\n` +
          `🎮 **Lobby:**\n` +
          `\`\`\`\n${steamLink}\n\`\`\``,
      )

      .setAuthor({
        name: `${challenger.username} inició un desafío`,
